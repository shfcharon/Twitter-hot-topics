"""
资本科技热点 Pipeline V1 Topic-Focused（话题聚焦版本）
基于 V1 Advanced 的进一步优化：

核心改进：
1. 话题提取增强：
   - 从推文内容中提取真实讨论话题（事件、发明、影响等）
   - 识别中心推文（被大量转发的推文）作为话题核心
   - 使用规则和简单的NLP来总结话题

2. 标题和描述生成优化：
   - 不再只是关键词列表，而是反映真实讨论话题
   - 生成类似"AI公司因版权问题被起诉"这样的标题
   - 描述说明"发生了什么"、"有什么影响"

3. 保留 V1 Advanced 的所有功能：
   - 语义聚类
   - 内容关键词提取
   - 趋势预测
"""
...
def step_e_topic_candidates(
    ...
    # E.5 内容关键词提取（优化版本新增）
    # 从推文内容中提取关键词作为候选主题
    print("  Extracting keywords from tweet content...")
    content_keywords_count = defaultdict(int)
    content_keywords_tweets = defaultdict(list)
    ...
    for tweet in tweets_24h_clean:
        ...
        # 提取关键词
        keywords = extract_keywords_from_text(text, min_freq=1)
        for keyword in keywords:
            # 检查是否与资本科技相关
            if filter_capital_tech:
                if not is_capital_tech_related(keyword):
                    continue
            
            content_keywords_count[keyword] += 1
            content_keywords_tweets[keyword].append(tweet.get("tweet_id", ""))
    
    # 只保留出现频率 >= 2 的关键词
    for keyword, count in content_keywords_count.items():
        if count >= 2:
            # 为每个关键词创建一个候选主题
            candidates.append({
                "topic_type": "content_keyword",
                "topic_key": keyword,
                "tweet_id": content_keywords_tweets[keyword][0] if content_keywords_tweets[keyword] else "",
                "author_username": "",
                "created_at": "",
                "local_date": "",
            })
    ...
    print(f"  Content keywords extracted: {len([c for c in candidates if c.get('topic_type') == 'content_keyword'])}")
    print("Step E: 完成\n")
...
def step_f_prime_semantic_clustering(
    ...
    if not use_clustering or len(topics) < 2:
        ...
        save_jsonl(semantic_topics, os.path.join(output_dir, "semantic_topics_24h_topic_focused.jsonl"))
        print("Step F': 完成（未聚类）\n")
    ...
    if SKLEARN_AVAILABLE:
        try:
            ...
            # DBSCAN 聚类（优化：使用更严格的 eps 值，使聚类更精确）
            # eps: 距离阈值（更小的值 = 更严格的聚类）
            # min_samples: 最小样本数
...
# ============================================================================
# 话题提取辅助函数（Topic-Focused 新增）
# ============================================================================

def extract_topic_from_tweets(
    tweets: List[dict],
    max_tweets: int = 10
) -> Dict[str, Any]:
    """
    ...
    Args:
        tweets: 推文列表（已按热度排序）
        max_tweets: 最多分析的推文数量
    
    Returns:
        包含话题信息的字典：
        - topic_title: 话题标题（如"AI公司因版权问题被起诉"）
        - topic_summary: 话题摘要（发生了什么）
        - key_entities: 关键实体（公司、产品、人物等）
        - event_type: 事件类型（invention/announcement/controversy/trend等）
    """
    if not tweets:
        return {
            "topic_title": "未知话题",
    ...
    # 分析前N条最热门的推文
    analyzed_tweets = tweets[:max_tweets]
    all_texts = [t.get("text", "") or t.get("clean_text", "") for t in analyzed_tweets]
    
    # 提取关键实体（公司名、产品名等）
    key_entities = set()
    event_keywords = {
        "invention": ["invent", "create", "develop", "build", "launch", "release", "new"],
        "announcement": ["announce", "reveal", "unveil", "introduce", "present"],
        "controversy": ["sue", "lawsuit", "legal", "copyright", "dispute", "controversy", "criticize"],
        "funding": ["funding", "raise", "investment", "IPO", "valuation", "acquire", "acquisition"],
        "trend": ["trend", "growing", "increasing", "popular", "rising"],
        "breakthrough": ["breakthrough", "milestone", "achievement", "success", "record"],
    }
    ...
    # 确定主要事件类型
    if event_scores:
        main_event_type = max(event_scores.items(), key=lambda x: x[1])[0]
    else:
        main_event_type = "discussion"
    
    # 提取公司/产品名（简单规则：大写字母开头的词、常见公司名）
    common_companies = [
        "OpenAI", "Anthropic", "Google", "Microsoft", "Apple", "Meta", "Amazon",
        "Tesla", "NVIDIA", "AMD", "Intel", "Twitter", "X", "Elon", "Musk"
    ]
    ...
    for text in all_texts:
        ...
        for company in common_companies:
            if company.lower() in text.lower():
                key_entities.add(company)
        
        # 查找大写字母开头的词（可能是公司/产品名）
        words = re.findall(r'\b[A-Z][a-z]+\b', text)
        for word in words:
            if len(word) > 2 and word not in ["The", "This", "That", "What", "How", "Why"]:
                key_entities.add(word)
    
    # 生成话题标题
    # 策略1：如果有中心推文（被大量转发），使用其核心内容
    # 策略2：从多条推文中提取共同主题
    
    # 找到最热门的推文作为核心
    core_tweet = analyzed_tweets[0] if analyzed_tweets else None
    core_text = (core_tweet.get("text", "") or core_tweet.get("clean_text", "")) if core_tweet else ""
    
    # 简化文本（去除URL、多余空格）
    core_text_clean = re.sub(r'https?://\S+', '', core_text)
    core_text_clean = re.sub(r'\s+', ' ', core_text_clean).strip()
    
    # 提取关键句子（前100个字符，通常是推文的核心）
    if core_text_clean:
        key_sentence = core_text_clean[:150]
        # 尝试找到句子的完整部分
        if len(core_text_clean) > 150:
            # 找到最后一个句号或问号
            last_punct = max(
                core_text_clean[:150].rfind('.'),
                core_text_clean[:150].rfind('?'),
                core_text_clean[:150].rfind('!')
            )
            if last_punct > 50:
                key_sentence = core_text_clean[:last_punct + 1]
    else:
        key_sentence = "讨论热点话题"
    
    # 生成话题标题（简化版：使用关键句子或关键实体）
    if key_entities:
        entities_str = ", ".join(list(key_entities)[:3])
        if main_event_type == "invention":
            topic_title = f"{entities_str} 发布新产品/技术"
        elif main_event_type == "announcement":
            topic_title = f"{entities_str} 发布重要公告"
        elif main_event_type == "controversy":
            topic_title = f"{entities_str} 相关争议/法律问题"
        elif main_event_type == "funding":
            topic_title = f"{entities_str} 融资/收购动态"
        elif main_event_type == "breakthrough":
            topic_title = f"{entities_str} 取得重大突破"
        else:
            # 使用关键句子的简化版
            if len(key_sentence) > 60:
                topic_title = key_sentence[:60] + "..."
            else:
                topic_title = key_sentence
    else:
        # 没有关键实体，使用关键句子
        if len(key_sentence) > 80:
            topic_title = key_sentence[:80] + "..."
        else:
            topic_title = key_sentence
    
    # 生成话题摘要
    if len(analyzed_tweets) > 1:
        summary = f"多位 KOL 讨论：{key_sentence[:100]}"
    else:
        summary = key_sentence[:150]
    
    return {
        "topic_title": topic_title,
        "topic_summary": summary,
        "key_entities": list(key_entities)[:5],
        "event_type": main_event_type,
        "core_tweet_text": key_sentence[:200]
    }


# ============================================================================
# Step G' - 基于 Semantic Topic 的热度评分与 Top5 (V1 新增，Topic-Focused 增强版)
# ============================================================================
...
def step_g_prime_semantic_hot_score(
    ...
    # 为每个 semantic topic 添加可读信息（Topic-Focused 增强版）
    for i, st in enumerate(top5, 1):
        ...
        analyzed_tweets = [tweet for _, _, tweet in tweet_with_metrics[:10]]
        
        # 从推文内容中提取真实话题（Topic-Focused 核心功能）
        topic_info = extract_topic_from_tweets(analyzed_tweets, max_tweets=10)
        
        # 生成标题：使用提取的话题标题（而不是简单关键词）
        if topic_info["topic_title"] and topic_info["topic_title"] != "未知话题":
            st["title"] = topic_info["topic_title"]
        elif keywords:
            # 如果没有提取到话题，回退到关键词（但尽量简化）
            title_keywords = keywords[:2]
            if len(keywords) > 2:
                st["title"] = f"{', '.join(title_keywords)} 等话题讨论"
            else:
                st["title"] = f"{', '.join(title_keywords)} 相关讨论"
        else:
            st["title"] = f"热点话题 #{i}（{unique_author_cnt} 位 KOL 参与）"
        
        # 生成描述：说明发生了什么、有什么影响（而不是简单统计）
        if topic_info["topic_summary"] and topic_info["topic_summary"] != "暂无信息":
            # 使用提取的话题摘要
            core_summary = topic_info["topic_summary"]
            if len(core_summary) > 200:
                core_summary = core_summary[:200] + "..."
            
            # 添加统计信息
            if unique_author_cnt > 1:
                st["description"] = f"{unique_author_cnt} 位 KOL 参与讨论，共 {tweet_cnt} 条推文。核心内容：{core_summary}"
            else:
                st["description"] = f"{unique_author_cnt} 位 KOL 参与讨论，共 {tweet_cnt} 条推文。{core_summary}"
        else:
            # 回退到统计信息
            if unique_author_cnt > 1:
                st["description"] = f"{unique_author_cnt} 位 KOL 参与讨论，共 {tweet_cnt} 条推文，引发广泛关注"
            else:
        ...
        st["topic_info"] = {
            ...
            "key_entities": topic_info["key_entities"],
            "core_tweet_text": topic_info.get("core_tweet_text", "")
        }
    ...
    result = {
        ...
        "window": "24h",
        "version": "V1 Topic-Focused (话题聚焦版本)",
        "top5": top5,
    }
    
    output_path = os.path.join(output_dir, "hot_topics_top5_v1_topic_focused.json")
...
def step_h_prime_growth_prediction(
    ...
    save_jsonl(predictions, os.path.join(output_dir, "growth_predictions_24h_topic_focused.jsonl"))
...
def step_i_daily_report(
    ...
    for topic in hot_topics_top5:
        # 对于 semantic topics，可能没有 topic_type/topic_key，使用 semantic_topic_id
        topic_type = topic.get("topic_type", "")
        topic_key = topic.get("topic_key", "")
        semantic_topic_id = topic.get("semantic_topic_id", "")
        
        # 尝试匹配趋势（如果有 member_topics，尝试匹配第一个）
        trend = None
        if topic_type and topic_key:
            trend = trend_map.get((topic_type, topic_key))
        elif semantic_topic_id:
            # 对于 semantic topic，尝试从 member_topics 中匹配
            member_topics = topic.get("member_topics", [])
            if member_topics:
                first_member = member_topics[0]
                trend = trend_map.get((first_member.get("topic_type", ""), first_member.get("topic_key", "")))
        
        # 构建易读的热点信息
        hot_topic = {
            "rank": topic.get("rank", 0),
            "title": topic.get("title", topic_key or semantic_topic_id or "未知话题"),
            "description": topic.get("description", ""),
            "type": topic_type or "semantic_topic",
            "semantic_topic_id": semantic_topic_id,
            "hot_score": round(topic.get("hot_score", 0), 2),
            "topic_info": topic.get("topic_info", {}),  # Topic-Focused 新增：话题提取信息
            "metrics": {
        ...
        if growth_predictions:
            ...
            if topic_type and topic_key:
                ...
                if member_topics:
                    first_member = member_topics[0]
                    prediction = prediction_map.get((first_member.get("topic_type", ""), first_member.get("topic_key", "")))
    ...
    output_path = os.path.join(output_dir, "daily_report_v1_topic_focused.json")
    with open(output_path, "w", encoding="utf-8") as f:
    ...
    readable_path = os.path.join(output_dir, "daily_report_readable_v1_topic_focused.json")
    with open(readable_path, "w", encoding="utf-8") as f:
        json.dump(readable_report, f, ensure_ascii=False, indent=2)
    
    print("Step I: 完成")
    print(f"  生成文件: daily_report_v1_topic_focused.json, daily_report_readable_v1_topic_focused.json\n")
...
def run_pipeline(
    ...
    """
    print("=" * 60)
    print("资本科技热点 Pipeline V1 (Topic-Focused)")
    print("=" * 60)
    ...
    if use_v1_features and use_semantic_clustering:
        print("  - semantic_topics_24h_topic_focused.jsonl (V1)")
        print("  - hot_topics_top5_v1_topic_focused.json (V1 Topic-Focused)")
    else:
        print("  - hot_topics_top5.json")
    print("  - trends_14d.jsonl")
    if use_v1_features and use_growth_prediction:
        print("  - growth_predictions_24h_topic_focused.jsonl (V1)")
    if use_v1_features:
        print("  - daily_report_v1_topic_focused.json (V1 Advanced)")
        print("  - daily_report_readable_v1_topic_focused.json (V1 Advanced)")
    else:
