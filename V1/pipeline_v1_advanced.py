"""
资本科技热点 Pipeline V1 Advanced（优化版本）
基于 V1 的进一步优化：

优化内容：
1. 聚类算法优化：
   - 调整相似度阈值（更严格的聚类）
   - 改进 embedding 方法（更好的 TF-IDF 向量化）
   - 使用更智能的聚类策略
...
def step_f_prime_semantic_clustering(
    ...
    if not use_clustering or len(topics) < 2:
        ...
        save_jsonl(semantic_topics, os.path.join(output_dir, "semantic_topics_24h_advanced.jsonl"))
        print("Step F': 完成（未聚类）\n")
...
def run_pipeline(
    ...
    if use_v1_features and use_semantic_clustering:
        print("  - semantic_topics_24h_advanced.jsonl (V1)")
        print("  - hot_topics_top5_v1_advanced.json (V1 Advanced)")
    else:
        print("  - hot_topics_top5.json")
    print("  - trends_14d.jsonl")
    if use_v1_features and use_growth_prediction:
        print("  - growth_predictions_24h_advanced.jsonl (V1)")
    if use_v1_features:
        print("  - daily_report_v1_advanced.json (V1 Advanced)")
        print("  - daily_report_readable_v1_advanced.json (V1 Advanced)")
    else:
