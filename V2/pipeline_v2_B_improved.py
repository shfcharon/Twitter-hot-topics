...
def step_c_etl_simplified(
    ...
    save_jsonl(tweets_14d_clean, os.path.join(output_dir, "tweets_14d_clean_v2B_improved.jsonl"))
    save_jsonl(tweets_24h_clean, os.path.join(output_dir, "tweets_24h_clean_v2B_improved.jsonl"))
...
def step_d_propagation_edges_simplified(
    ...
    save_jsonl(edges_14d, os.path.join(output_dir, "edges_14d_v2B_improved.jsonl"))
    save_jsonl(edges_24h, os.path.join(output_dir, "edges_24h_v2B_improved.jsonl"))
...
def extract_keywords_from_text_improved(text: str, min_length: int = 4) -> List[str]:
    """从推文文本中提取关键词（改进版）"""
    if not text:
        return []
    
    # 提取单词
    words = re.findall(r'\b[A-Z][a-z]+\b|\b[A-Z]{2,}\b', text)  # 提取大写开头的词或全大写词（可能是产品/技术名）
    ...
    tech_products = [
        ...
        'Transformer', 'BERT', 'T5', 'PaLM', 'LaMDA', 'DALL-E', 'Midjourney', 'Stable Diffusion',
        'TensorFlow', 'PyTorch', 'JAX', 'CUDA', 'TPU', 'GPU', 'CPU', 'H100', 'A100', 'Blackwell',
        'OpenAI', 'Anthropic', 'Google', 'Microsoft', 'Apple', 'Meta', 'Amazon', 'Tesla', 'NVIDIA', 'AMD', 'Intel',
    ...
    for word in words:
        if len(word) >= min_length and word not in stop_words and word not in keywords:
            # 检查是否是常见的技术术语
            if any(term.lower() in word.lower() for term in ['AI', 'ML', 'LLM', 'GPT', 'API', 'SDK', 'GPU', 'CPU']):
                keywords.append(word)
            # 检查是否是首字母大写的专有名词（可能是产品名）
...
def extract_all_entities(tweet: dict) -> Dict[str, List[str]]:
    ...
    keywords = extract_keywords_from_text_improved(text_original)
    
    # 分类关键词为产品和技术
    tech_keywords = ['GPT', 'LLM', 'AI', 'ML', 'AGI', 'NLP', 'CV', 'RL', 'Transformer', 'BERT', 'T5',
                     'TensorFlow', 'PyTorch', 'JAX', 'CUDA', 'TPU', 'GPU', 'CPU', 'API', 'SDK',
                     'ChatGPT', 'Claude', 'Gemini', 'LLaMA', 'Mistral', 'DALL-E', 'Midjourney']
    
    for keyword in keywords:
        if any(tech in keyword.upper() for tech in tech_keywords):
...
def step_f_community_detection(
    ...
    save_jsonl(communities_info, os.path.join(output_dir, "communities_24h_v2B_improved.jsonl"))
...
def find_community_center(graph: HeterogeneousGraph, community_nodes: List[str], tweet_map: Dict[str, dict] = None) -> Tuple[Optional[str], Optional[str], List[str]]:
    ...
    if tweet_map:
        for node_key in community_nodes:
            if node_key.startswith("tweet:"):
                ...
                if tweet:
                    text = tweet.get("text", "")
                    extracted = extract_keywords_from_text_improved(text)
    ...
    for kw in keywords:
        keyword_freq[kw] += 1
    keywords = sorted(keyword_freq.items(), key=lambda x: x[1], reverse=True)[:5]
...
def step_g_community_scoring(
    ...
    result = {
        ...
        "window": "24h",
        "version": "V2-B Improved (Discussion Graph + Community Detection - Enhanced)",
        "top5": top5,
    }
    
    output_path = os.path.join(output_dir, "hot_communities_top5_v2B_improved.json")
...
def step_i_community_presentation(
    ...
    for comm in top5_communities:
        ...
        if not title_parts and center_tweet_id:
            ...
            if center_tweet:
                ...
                extracted_kw = extract_keywords_from_text_improved(first_sentence)
                if extracted_kw:
    ...
    output_path = os.path.join(output_dir, "daily_report_v2B_improved.json")
    with open(output_path, "w", encoding="utf-8") as f:
    ...
    readable_report = {
        ...
        "时间窗口": "24小时",
        "版本": "V2-B Improved (讨论图谱 + 社区发现版本 - 改进版)",
        "Top5 热点议题": [
            {
                "排名": c["rank"],
                "议题": c["title"],
    ...
    readable_path = os.path.join(output_dir, "daily_report_readable_v2B_improved.json")
    with open(readable_path, "w", encoding="utf-8") as f:
        json.dump(readable_report, f, ensure_ascii=False, indent=2)
    
    print("Step I: 完成")
    print(f"  生成文件: daily_report_v2B_improved.json, daily_report_readable_v2B_improved.json\n")
...
def run_pipeline(
    ...
    """
    print("=" * 60)
    print("资本科技热点 Pipeline V2-B Improved (Discussion Graph + Community Detection - Enhanced)")
    print("=" * 60)
    print()
    ...
    print(f"\n输出文件目录: {output_dir}")
    print("\n主要输出文件:")
    print("  - tweets_14d_clean_v2B_improved.jsonl")
    print("  - tweets_24h_clean_v2B_improved.jsonl")
    print("  - edges_14d_v2B_improved.jsonl")
    print("  - edges_24h_v2B_improved.jsonl")
    print("  - communities_24h_v2B_improved.jsonl (V2-B Improved)")
    print("  - hot_communities_top5_v2B_improved.json (V2-B Improved)")
    print("  - daily_report_v2B_improved.json (V2-B Improved)")
    print("  - daily_report_readable_v2B_improved.json (V2-B Improved)")
