"""
资本科技热点 Pipeline V2-B（讨论图谱 + 社区发现版本）
基于 V2 的 V2-B 方案实现：
...
- Tweet
- Author（KOL）
- Entity（公司/产品/人名/ticker/hashtag/domain/关键短语）
...
- 使用简化的 Louvain 算法或基于连通分量的方法
- 输出社区 = 议题 cluster
- 社区的"议题标题"来自：社区内最中心的实体节点 + 最中心的 tweet
"""
...
def step_c_etl_simplified(
    ...
    save_jsonl(tweets_14d_clean, os.path.join(output_dir, "tweets_14d_clean_v2B.jsonl"))
    save_jsonl(tweets_24h_clean, os.path.join(output_dir, "tweets_24h_clean_v2B.jsonl"))
...
def step_d_propagation_edges_simplified(
    ...
    save_jsonl(edges_14d, os.path.join(output_dir, "edges_14d_v2B.jsonl"))
    save_jsonl(edges_24h, os.path.join(output_dir, "edges_24h_v2B.jsonl"))
...
def step_f_community_detection(
    ...
    save_jsonl(communities_info, os.path.join(output_dir, "communities_24h_v2B.jsonl"))
...
def find_community_center(graph: HeterogeneousGraph, community_nodes: List[str]) -> Tuple[Optional[str], Optional[str]]:
    """
    找到社区的中心节点
    返回：(最中心的实体节点, 最中心的 tweet 节点)
    """
    # 计算每个节点的中心度（基于连接数）
    ...
    return center_entity, center_tweet
...
def step_g_community_scoring(
    ...
    for comm_id, nodes in community_nodes.items():
        ...
        # 找到社区中心
        center_entity, center_tweet = find_community_center(graph, nodes)
    ...
    result = {
        ...
        "window": "24h",
        "version": "V2-B (Discussion Graph + Community Detection)",
        "top5": top5,
    }
    
    output_path = os.path.join(output_dir, "hot_communities_top5_v2B.json")
...
def step_i_community_presentation(
    ...
    output_path = os.path.join(output_dir, "daily_report_v2B.json")
    with open(output_path, "w", encoding="utf-8") as f:
    ...
    readable_path = os.path.join(output_dir, "daily_report_readable_v2B.json")
    with open(readable_path, "w", encoding="utf-8") as f:
        json.dump(readable_report, f, ensure_ascii=False, indent=2)
    
    print("Step I: 完成")
    print(f"  生成文件: daily_report_v2B.json, daily_report_readable_v2B.json\n")
...
def run_pipeline(
    ...
    """
    print("=" * 60)
    print("资本科技热点 Pipeline V2-B (Discussion Graph + Community Detection)")
    print("=" * 60)
    print()
    ...
    print(f"\n输出文件目录: {output_dir}")
    print("\n主要输出文件:")
    print("  - tweets_14d_clean_v2B.jsonl")
    print("  - tweets_24h_clean_v2B.jsonl")
    print("  - edges_14d_v2B.jsonl")
    print("  - edges_24h_v2B.jsonl")
    print("  - communities_24h_v2B.jsonl (V2-B)")
    print("  - hot_communities_top5_v2B.json (V2-B)")
    print("  - daily_report_v2B.json (V2-B)")
    print("  - daily_report_readable_v2B.json (V2-B)")
